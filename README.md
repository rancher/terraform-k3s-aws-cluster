...
|------|-------------|------|---------|:-----:|
| agent\_image\_id | AMI to use for k3s agent instances | `string` | n/a | yes |
...
| agent\_k3s\_exec | exec args to pass to k3s agents | `string` | n/a | yes |
| agent\_node\_count | Number of agent nodes to launch | `number` | `3` | no |
| aws\_azs | List of AWS Availability Zones in the VPC | `list` | n/a | yes |
| aws\_profile | Name of the AWS Profile to use for authentication | `string` | n/a | yes |
...
| db\_name | Name of database to create in RDS | `string` | n/a | yes |
...
| k3s\_tls\_san | Sets k3s tls-san flag to this value instead of the default load balancer | `string` | n/a | yes |
...
| rancher2\_token\_key | Rancher2 API token for authentication | `string` | n/a | yes |
...
| server\_image\_id | AMI to use for k3s server instances | `string` | n/a | yes |
...
| server\_k3s\_exec | exec args to pass to k3s server | `string` | n/a | yes |
...
| subdomain | subdomain to host rancher on, instead of using `var.name` | `string` | n/a | yes |
| use\_route53 | Configures whether to use route\_53 DNS or not | `bool` | `true` | no |
| vpc\_id | The vpc id that Rancher should use | `string` | n/a | yes |
